...
export class VideoComponent implements OnInit{
  ...
  videos: any[] = []; // Array to store video objects

  constructor(private mainserve:MainService,private route:ActivatedRoute){}
  ngOnInit(): void {
  ...
  // loadsection(){
  //   this.mainserve.sectionbysectionId(this.sectionId).subscribe((res:any)=>{
  //     this.videos=res
  ...
  loadSectionContent(): void {
    this.mainserve.sectionbysectionId(this.sectionId!).subscribe(
      (res: any) => {
        if (res && res.content && res.content.video) {
          // If the API returns a single video, wrap it in an array
          this.videos = Array.isArray(res.content.video) ? res.content.video : [res.content.video];
          console.log('Loaded videos:', this.videos);
        } else {
          console.warn('No videos found for this section.');
        }
      },
      (error: any) => {
